...
const DrawerMenu = (() => {
  ...
  function build() {
    ...
    _drawer.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:20px 20px 16px;">
        <div style="display:flex;align-items:center;gap:12px;">
          ...
          <span style="font-family:var(--font-display);font-weight:700;font-size:1.25rem;letter-spacing:-0.03em;">Nuckleo</span>
        </div>
        <button id="drawer-avatar-btn" style="
          ...
          overflow:hidden;flex-shrink:0;font-family:var(--font-body);
        ">?</button>
      </div>
      ...
      <nav style="flex:1;padding:8px 12px;overflow-y:auto;">
        <a class="drawer-item active" data-page="dashboard">
          <span class="drawer-item__icon">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2"/><rect x="8.5" y="8.5" width="7" height="7" rx="1.5" fill="currentColor"/></svg>
          </span>
          <span>Root</span>
        </a>
        <a class="drawer-item" data-page="spaces">
          <span class="drawer-item__icon">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><circle cx="7.5" cy="7.5" r="3"/><circle cx="16.5" cy="7.5" r="3"/><circle cx="7.5" cy="16.5" r="3"/><circle cx="16.5" cy="16.5" r="3"/></svg>
          </span>
          <span>Ctrl+S</span>
        </a>
        <a class="drawer-item" data-page="favorites">
          <span class="drawer-item__icon">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C12 2 13.5 8.5 15 10C16.5 11.5 23 12 23 12C23 12 16.5 12.5 15 14C13.5 15.5 12 22 12 22C12 22 10.5 15.5 9 14C7.5 12.5 1 12 1 12C1 12 7.5 11.5 9 10C10.5 8.5 12 2 12 2Z"/></svg>
          </span>
          <span>Xodós</span>
        </a>
      ...
      <div style="padding:16px 20px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;">
        ...
        <button id="drawer-color-dot" style="
          ...
          cursor:pointer;transition:transform 0.15s;padding:2px;
          display:flex;align-items:center;justify-content:center;background:transparent;
        ">
          <svg width="28" height="28" viewBox="0 0 28 28">
            <circle cx="9.5" cy="9.5" r="6.5" fill="#FF5F57" opacity="0.9"/>
            <circle cx="18.5" cy="9.5" r="6.5" fill="#FFBD2E" opacity="0.9"/>
            <circle cx="9.5" cy="18.5" r="6.5" fill="#28CA42" opacity="0.9"/>
            <circle cx="18.5" cy="18.5" r="6.5" fill="#0D72F6" opacity="0.9"/>
          </svg>
        </button>
      </div>
    `;
    ...
    style.textContent = `
      .drawer-item {
        ...
        text-decoration:none;margin-bottom:2px;font-family:var(--font-body);
      }
      .drawer-item:hover { background:var(--bg-hover);color:var(--text-primary); }
      .drawer-item.active { background:var(--bg-active);color:var(--accent); }
      .drawer-item--danger { color:#FF4444 !important; }
      .drawer-item--danger:hover { background:rgba(255,68,68,0.08) !important; }
  ...
  function attachListeners() {
    ...
    document.getElementById('drawer-theme')?.addEventListener('click', () => {
      ...
      document.getElementById('main-app')?.classList.add('theme-panel-open');
    });
...
const Modal = (() => {
  ...
  function openEditSpace(space) {
    ...
    _selectedColor = space.coverColor || '#A7F24A';
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div class="modal">
          <div class="modal__header">
            ...
            <button class="btn-icon modal-close-profile">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
          <div class="modal__body">
            <div class="field"><label class="field__label">Nome</label><input type="text" class="input" id="profile-name" placeholder="Seu nome" /></div>
            <div class="field"><label class="field__label">Username</label><input type="text" class="input" id="profile-username" placeholder="@username" /></div>
            <div class="field"><label class="field__label">Bio</label><textarea class="input textarea" id="profile-bio" rows="3" placeholder="Conte um pouco sobre você..." maxlength="300"></textarea></div>
            <div class="field"><label class="field__label">Website</label><input type="url" class="input" id="profile-website" placeholder="https://..." /></div>
            <div class="field"><label class="field__label">URL do avatar</label><input type="url" class="input" id="profile-avatar" placeholder="https://..." /></div>
          </div>
          <div class="modal__footer">
            <button class="btn btn--ghost modal-close-profile">Cancelar</button>
            <button class="btn btn--primary" id="btn-save-profile">Salvar</button>
          </div>
        </div>`;
      document.body.appendChild(modal);

      modal.querySelectorAll('.modal-close-profile').forEach(btn => {
      ...
      document.getElementById('btn-save-profile')?.addEventListener('click', async () => {
        ...
        try {
          ...
          const updated = { ...State.get('user'), ...res.data };
          State.set('user', updated);
          const nameEl = document.getElementById('user-name');
          const handleEl = document.getElementById('user-handle');
          if (nameEl) nameEl.textContent = updated.name || name;
          if (handleEl) handleEl.textContent = username ? `@${username}` : updated.email || '';
          modal.style.display = 'none';
...
const App = (() => {
  ...
  function navigateTo(page) {
    ...
    Helpers.qsa('.bottom-nav__item').forEach(n => {
      n.classList.toggle('active', n.dataset.page === page);
    });
  ...
  function updateUserUI(user) {
    ...
    if (user.avatar) {
      ...
      if (avatarInit) {
        ...
        avatarInit.textContent = Helpers.initials(user.name || '?');
      }
    }

    // Bottom nav avatar
    const bottomAvatar = document.getElementById('bottom-avatar');
    const bottomInitials = document.getElementById('bottom-avatar-initials');
    if (user.avatar) {
      if (bottomAvatar) { bottomAvatar.src = user.avatar; bottomAvatar.style.display = 'block'; }
      if (bottomInitials) bottomInitials.style.display = 'none';
    } else {
      if (bottomAvatar) bottomAvatar.style.display = 'none';
      if (bottomInitials) {
        bottomInitials.style.display = 'flex';
        bottomInitials.textContent = Helpers.initials(user.name || '?');
      }
    }
  ...
  // Mantida para não quebrar referências internas — UI desativada
  ...
  function initTheme() {
    ...
    document.getElementById('btn-open-theme')?.addEventListener('click', () => {
      ...
      document.getElementById('main-app')?.classList.toggle('theme-panel-open');
    });
  ...
  function renderSearchResults({ users = [], spaces = [], items = [] }, container) {
    ...
    container.querySelectorAll('[data-space-id]').forEach(el => {
      el.addEventListener('click', () => {
        ...
        SpaceDetailPage.open(el.dataset.spaceId);
      });
    });
    container.querySelectorAll('[data-item-id]').forEach(el => {
      el.addEventListener('click', () => {
        document.getElementById('modal-search').style.display = 'none';
        Modal.openItemDetail(el.dataset.itemId);
      });
    });
  }
  ...
  function init() {
    ...
    // Bottom nav (mobile)
    Helpers.qsa('.bottom-nav__item[data-page]').forEach(item => {
      item.addEventListener('click', () => navigateTo(item.dataset.page));
    });

    // Avatar sidebar → abre drawer
    document.getElementById('user-chip')?.addEventListener('click', () => DrawerMenu.toggle());

    // Avatar bottom nav → abre drawer
    document.getElementById('bottom-nav-profile')?.addEventListener('click', () => DrawerMenu.toggle());

    // Outros botões
    Helpers.qsa('[data-page]').forEach(el => {
      if (!el.classList.contains('nav-item') && !el.classList.contains('bottom-nav__item')) {
    ...
    document.getElementById('btn-bgcolor')?.addEventListener('click', () => {
      document.getElementById('theme-panel')?.classList.add('open');
      document.getElementById('main-app')?.classList.add('theme-panel-open');
    });
  ...
  return { init, navigateTo, handleAuthSuccess, logout, applyAccent };
})();
