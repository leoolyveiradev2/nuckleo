// Tela de login/registro - Placeholder
/* pages/auth.js */
...
const AuthPage = (() => {
  ...
  function clearErrors() {
    Helpers.qsa('.auth-error').forEach(el => { el.textContent = ''; });
  }
  ...
  function initLogin() {
    ...
    btn.addEventListener('click', async () => {
      ...
      if (!email || !password) return setError('auth-error', 'Preencha todos os campos.');
      setLoading(btn, true);
      try {
  ...
  function initRegister() {
    ...
    btn.addEventListener('click', async () => {
      ...
      if (!name || !email || !password) return setError('reg-error', 'Preencha todos os campos.');
      if (password.length < 8)          return setError('reg-error', 'Senha deve ter pelo menos 8 caracteres.');
  ...
  function initGoogleOAuth() {
    ['btn-google-login', 'btn-google-register'].forEach(id => {
      document.getElementById(id)?.addEventListener('click', () => {
        Helpers.toast('Google OAuth requer configuração no .env', 'error', 4000);
      });
    });
  }
